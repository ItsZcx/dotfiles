...
/**
 * Git push guardrail — Pi may commit when told to, but must NEVER push.
 *
 ...
 *
 * Commits are governed by a soft, instructional rule rather than a hard block:
 * Pi must not commit on its own initiative, only when the user tells it to.
 * That rule lives in ~/.pi/agent/APPEND_SYSTEM.md (the standing instruction),
 * and the /commit prompt template is the canonical way to ask. Because it is
 * behavioural, any clear user instruction ("commit this", "go ahead and
 * commit") works — the guardrail deliberately does not second-guess intent.
 *
  ...
  pi.on("tool_call", async (event) => {
    ...
    if (containsSubcommand(raw, "push")) {
      return {
        block: true,
        reason:
          "Blocked by git guardrail: Pi may NEVER push. Push manually from your own terminal.",
      };
    }
  });
}
